...
class FlightSchedule {
    ...
    getFlightsForDate(date) {
        ...
        return this.scheduleData.flights.filter(flight => flight.date === dateStr);
    }
    ...
    renderPilotInfo() {
        ...
        document.title = `${pilot.displayName} - Flight Schedule`;
                ...
                <div class="day-card ${isToday ? 'today' : ''}">
                    ...
                    <div class="day-content">
                        ${flights.length > 0 ?
                            ...
                            '<p class="no-flight">No flights scheduled</p>'
                        }
                    </div>
                </div>
            `;
    ...
    renderFlightItem(flight) {
        const formattedTime = this.formatTime(flight.departureTime);
        const { color, icon } = this.getCategoryConfig(flight.category);

        return `
            <div class="flight-item" style="border-left-color: ${color}">
